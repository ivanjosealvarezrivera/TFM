<template>
  <div class="bg-white dark:bg-brand-gray-950 rounded-2xl shadow-sm border border-brand-gray-100 dark:border-brand-gray-800 overflow-hidden transition-colors duration-300">
    <div class="p-6 border-b border-brand-gray-50 dark:border-brand-gray-800 flex justify-between items-center bg-brand-gray-50/50 dark:bg-brand-gray-800/50">
      <h3 class="text-lg font-bold text-brand-gray-700 dark:text-brand-gray-200">Detalle de Compras por Cliente</h3>
      <span class="text-sm text-brand-gray-500 dark:text-brand-gray-400 font-medium">{{ data.length }} clientes registrados</span>
    </div>
      ...
      <Column field="name" header="Nombre Cliente / NIF" sortable class="min-w-[300px]">
        <template #body="{ data }">
          <div class="flex flex-col">
            ...
            <span class="text-[10px] text-slate-500 dark:text-gray-400 font-mono">{{ data.nif }}</span>
          </div>
        </template>
      </Column>
